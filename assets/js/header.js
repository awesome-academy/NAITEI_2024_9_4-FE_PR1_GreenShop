fetch('/src/layouts/header.html')
  ...
  .then(async (data) => {
    ...
    if (currentPath.includes('/cart')) {
      if (!isLogin()) {
        showToast(loadLocalData(getLanguage()).alert.error_login_to_view_cart);
        setTimeout(() => {
...
const loadLocalData = async (currentLanguage) => {
  try {
    const response = await fetch(`../../assets/locales/${currentLanguage}.json`);
